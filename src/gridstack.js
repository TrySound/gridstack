...
export default class GridStack {
    constructor(el, opts) {
        const self = this;
        let oneColumnMode;
        ...
        const isNested = this.container.closest('.' + opts.itemClass).length > 0;
        ...
        const isAutoCellHeight = this.opts.cellHeight === 'auto';
        if (isAutoCellHeight) {
        ...
        if (typeof self.opts.removable === 'string') {
            const trashZone = $(self.opts.removable);
        ...
        if (self.opts.acceptWidgets) {
            ...
            const onDrag = function(event, ui) {
                ...
                const node = el.data('_gridstack_node');
                const pos = self.getCellFromPixel(ui.offset, true);
            ...
            .on(self.container, 'dropover', function(event, ui) {
                ...
                const origNode = el.data('_gridstack_node');

                const width = origNode ? origNode.width : (Math.ceil(el.outerWidth() / cellWidth));
                const height = origNode ? origNode.height : (Math.ceil(el.outerHeight() / cellHeight));

                draggingElement = el;

                const node = self.grid._prepareNode({width, height, _added: false, _temporary: true});
    ...
    _triggerChangeEvent(forceTrigger) {
        const elements = this.grid.getDirtyNodes();
    ...
    _updateStyles(maxHeight) {
        ...
        const getHeight = function(nbRows) {
    ...
    _updateContainerHeight() {
        ...
        const height = this.grid.getGridHeight();
    ...
    _prepareElementsByNode(el, node) {
        ...
        const dragOrResize = (event, ui) => {
            ...
            const lastTriedWidth = typeof width !== 'undefined' ? width : node.lastTriedWidth;
            const lastTriedHeight = typeof height !== 'undefined' ? height : node.lastTriedHeight;
        ...
        const onStartMoving = function(event) {
            self.container.append(self.placeholder);
            const o = $(this);
            self.grid.cleanNodes();
            self.grid.beginUpdate(node);
        ...
        const onEndMoving = function(event) {
            const o = $(this);
    ...
    willItFit(x, y, width, height, autoPosition) {
        return this.grid.canBePlacedWithRespectToHeight({x, y, width, height, autoPosition});
    }
    ...
    cellHeight(val, noUpdate) {
        if (typeof val === 'undefined') {
            ...
            const o = this.container.children('.' + this.opts.itemClass).first();
    ...
    getCellFromPixel(position, useOffset) {
        const containerPos = (typeof useOffset !== 'undefined' && useOffset) ?
